...
def compare_wdc_results_balance(dataSets_output_Dirpath, results_path, pseudo=False):
    directory = "Results/WDC/Balance/" + results_path
    ...
    for i in range(len(dataSets)):
        ...
        output = OutputPath + split_task[-3] + "/" + split_task[-2] + "/" + split_task[-1] + "/" + \
...
def write_results_balance(results_dict, folder_path, pseudo=False):
    if pseudo:
        output_path = folder_path + "/resultsPseudo.txt"
    else:
        output_path = folder_path + "/results.txt"
    ...
    # with open(output_path, "w") as file:
    #     for task, values in sorted(results_dict.items()):
    #         file.write(str(task))
    #         file.write("\n")
...
def write_AVG_task_results(results_dict, folder_path, pseudo=False, size=1):
    ...
    write_results_balance(results_dict, folder_path, pseudo)
    return


def wdcAVG():
    # directory = "Results/Magellan/NoiseRate" + str(noise_rate) + "/" + dataSets_output_Dirpath
    directory = "Results/WDC/Balance/"

    matcher = ["Bert", "SoftTFIDF"]
    loss = ["Noised", "Weighted"]
    #loss = ["Noised"]
    Thresholds = [0.8, 0.85, 0.9, 0.95, 0.97]
    pseudo = [False, True]
        ...
        for l in loss:
            ...
            for p in pseudo:
                ...
                for th in Thresholds:
                    output_dirPath = loss_path + "/" + m + str(th)
        ...
        for l in loss:
            for th in Thresholds:
                for seed in range(3):
                    ...
                    output_dirPath = m + str(th) + "/" + l + "/Seed" + str(seed)
                    results_path = l + "/" + m + str(th) + "/Seed" + str(seed)
                    for p in pseudo:
                        compare_wdc_results_balance(output_dirPath, results_path, p)

    return

if __name__ == '__main__':
    creatWDCBalanceResults()
    wdcAVG()
