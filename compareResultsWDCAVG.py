...
def wdcSeedsAVG(folder_path, pseudo=False):
    ...
    for seed in range(3):
        ...
        for i in range(0, len(lines), 2):
            ...
            # print(scores_dict)
            # print(len(scores_dict))
            if scores_dict['len'] != scores_dict['ones'] and scores_dict['acc'] > 0.5:
                if taskLine not in task_dict:
                    task_dict[taskLine] = scores_dict
...
def wdcAVG():
    ...
    directory = "Results/WDC/NoiseRate"
    noise_rate = [0.1, 0.2]
    #noise_rate = [0.1]
    ...
    for noise in noise_rate:
        for l in loss:
            loss_path = directory + str(noise) + "/" + l
            for p in pseudo:
                loss_dict = {}
                localMin_dict = {}
                for ds in DSNoiseSeed:
                    output_dirPath = loss_path + "/Counted" + str(ds)
                ...
                write_AVG_task_results(loss_dict, loss_path, p, size=len(DSNoiseSeed))
