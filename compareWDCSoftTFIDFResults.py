...
def compare_wdc_SoftTFIDF_results(SoftTFIDFThreshold, dataSets_output_Dirpath, result_path, pseudo=False):
    directory = "Results/WDC/SoftTFIDF/" + result_path
    print(directory)
    ...
    for i in range(len(dataSets)):
        ...
        output = OutputPath + split_task[-3] + "/" + split_task[-2] + "/" + split_task[-1] + "SoftTFIDF" + str(SoftTFIDFThreshold) + "/" + \
...
def wdcSoftTFIDFAVG(thresholds):

    directory = "Results/WDC/SoftTFIDF"
    # noise_rate = [0.1]
...
def create_SoftTFIDF_results(threshold):
    SoftTFIDFThreshold = threshold
    loss = ["Noised", "Weighted"]
    # loss = ["Weighted"]
    # DSNoiseSeed = [4]
    pseudo = [False, True]
    # task = ["results", "resultsPseudo"]
    for th in SoftTFIDFThreshold:
        for l in loss:
            for seed in range(3):
                # output_dirPath = "Weighted/Counted" + str(ds) + "/Seed" + str(seed)
                output_dirPath = l + "/Seed" + str(seed)
                result_path = l + "/Threshold" + str(th) + "/Seed" + str(seed)
                for p in pseudo:
                    compare_wdc_SoftTFIDF_results(th, output_dirPath, result_path, p)

if __name__ == '__main__':

    #SoftTFIDFThreshold = ["", 2, 3, 4, 5]
    SoftTFIDFThreshold = [0.5, 0.6, 0.7, 0.8, 0.75]

    create_SoftTFIDF_results(SoftTFIDFThreshold)
    wdcSoftTFIDFAVG(SoftTFIDFThreshold)
